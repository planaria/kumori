...
namespace kumori
{
	...
	public:
			...
			: mime_type_(std::move(mime_type))
		{
			...
			e_tag_ = encode_base64(hash_md5(content));
			last_modified_ = format_rfc1123(boost::posix_time::second_clock::universal_time());

			if (content_gzip.size() < content.size())
				encoded_contents_.push_back(std::make_pair(http_content_encoding::gzip, std::move(content_gzip)));

			if (content_deflate.size() < content.size())
				encoded_contents_.push_back(std::make_pair(http_content_encoding::deflate, std::move(content_deflate)));

			std::sort(encoded_contents_.begin(), encoded_contents_.end(),
				[](const encoded_content& lhs, const encoded_content& rhs)
			{
				return lhs.second.size() < rhs.second.size();
			});

			encoded_contents_.push_back(std::make_pair(http_content_encoding::identity, std::move(content)));
		}
